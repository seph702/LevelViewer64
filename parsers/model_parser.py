...
############################################
### NINTENDO 64 GRAPHICS PROCESSOR NOTES ###
############################################
...
if setfilesize:
    ...
    s_scale depends on fields 5 and 6:
    Textures.S_Scale = Convert.ToSingle((((CMD[5] << 4)| ((CMD[6] &0xF0) >> 4)) >> 2) + 1);

    t_scale depends on fields 6 and 7:
...
ddd speedrun    Vertex.U_Scale = Convert.ToSingle((CMD[4] << 8) | CMD[5]) / Convert.ToSingle(0xFFFF);
...
and then look for gsSPDisplayLists and get the addresses at the end of the name.
then go back in the file and look for:

static const Gfx ddd_seg7_dl_(address1)

this graphics list will contain gsSPVertex functions.  get address2 from the end of the ddd_seg7_dl_(address2).  then go find this vertex data earlier in the file

static const Vtx ddd_seg7_vertex_(address2)

the Vtx contains ( x, y, z ), unused bit, ( s, t ) texture coords, ( r, g, b, a ) texture colours
the Gfx 



finally, given U_scale, V_scale, S_scale, T_scale at a given time, we can calculate UV coordinates by:
U = S / (0x20) / Textures.S_Scale * U_Scale
V = T / (0x20) / Textures.T_Scale * V_Scale

note dividing by 0x20 is the same as bit shifting to the right by 5
also, S and T come in S10.5 notation by default.
